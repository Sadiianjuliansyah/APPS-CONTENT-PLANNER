...
function showPage(requestedPage) {
  ...
  document.title = `${config.title} | Content Planner`;
...
const STORAGE_KEY = "content-planner-data-v1";
...
function renderBackupPage() {
  ...
  importDescription.textContent =
    "Pilih file JSON hasil ekspor Content Planner. Data akan diganti setelah kamu menyetujui konfirmasi.";
...
function validateBackup(data) {
  if (
    ...
    throw new Error("File bukan backup Content Planner versi 1.");
  }
...
async function registerContentPlannerWorker() {
  ...
  try {
    ...
    if (existing) {
      ...
      if (hasOtherWorker) {
        console.warn(
          "Pendaftaran offline dibatalkan: alamat ini masih menggunakan service worker aplikasi lain. Gunakan profil Content Planner yang sudah terpisah."
        );
        return;
      }
    }
    ...
    console.log(
      "Service worker Content Planner terdaftar. Cek status activated di Application → Service Workers."
    );
  } catch (error) {
